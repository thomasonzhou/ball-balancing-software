...
Euler quat2euler(const Quaternion& q) {
  // All angles are negative wrt the axes viewed by the camera.
  // i.e. X roll (red) right, Y yaw (green) up, Z pitch (blue) towards viewer
  // we invert the green to switch to the frame of the IK
  Euler euler_angles;
  ...
  euler_angles.yaw = -sanity_clip(2 * atan2(sinp, cosp) - M_PI / 2);  // same as
...
void detectMarker() {
  ...
  const bool estimatePose = true;  // note: requires loading a calibration file
  ...
  const aruco::Dictionary dictionary =
      aruco::getPredefinedDictionary(ARUCOTAG_DICTIONARY);

  Mat camMatrix, distCoeffs;
  if (estimatePose) {
    // You can read camera parameters from tutorial_camera_params.yml
    bool readOk =
        readCameraParameters(CAMERA_PARAMS_FILE, camMatrix, distCoeffs);
    if (!readOk) {
  ...
  while (cam.getVideoFrame(image,1000)) {
    ...
    if (estimatePose && !ids.empty()) {
      ...
      for (size_t i = 0; i < nMarkers; i++) {
        solvePnP(objPoints, corners.at(i), camMatrix, distCoeffs, rvecs.at(i),
                 tvecs.at(i));
      }
    }
