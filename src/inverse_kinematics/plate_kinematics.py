...
def calculate_li(
    T: npt.NDArray, theta_y: float, phi_x: float, pi_plat: npt.NDArray, bi: npt.NDArray
) -> npt.NDArray:
...
if __name__ == "__main__":
    ...
    T = np.array(
        [0, 0, DEFAULT_PLATE_HEIGHT]
    )  # from the middle of the base of the motor mounts to the middle of the plate
    N = calculate_normal_from_dir_vec(np.array([0, 0, 0]), 0)  # at rest
    print(f"Default configuration: T-{T}, N-{N}")

    # From here, the pipeline is:
    motors = [
        Motor(orientation) for orientation in MOTOR_ORIENTATIONS
    ]  # initializes motors a, b, c
    ...
    for i, motor in enumerate(motors):
        ...
        print(
            f"Motor {i+1} angle: {np.round(abs_angle, 4)} rad, {np.round(abs_angle * 180 / np.pi, 0)} deg"
        )
